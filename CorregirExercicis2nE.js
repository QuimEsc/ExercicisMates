...
function Actualitzar() {
  if (sessionStorage.getItem("NomAlumnes") == null) { //En cas de no tindre nom d'alumnes.
    var data = {
      NomAlumne: ""
    }; // Datos a enviar, vacío inicialmente
    fetch(url, {
        method: 'POST',
    ...
    var data = {
      NomAlumne: NomAlumne
    }; // Datos a enviar con el nombre del alumno
    fetch(url, {
        method: 'POST',
        contentType: 'application/json',
        body: JSON.stringify(data)
      ...
      .then(function(response) {
        ...
        // ✅ CRIDA A MATHJAX DESPRÉS DE CARREGAR LES DADES DEL SERVIDOR
    ...
    // ✅ CRIDA A MATHJAX QUAN EL CONTINGUT JA ESTÀ AL LOCALSTORAGE
    MathJax.typesetPromise();
  }
}
...
function CrearDom() {
  ...
  // ✅ CRIDA A MATHJAX JUST DESPRÉS DE CREAR EL DOM
  MathJax.typesetPromise();
      ...
      .then(function(response) {
        ...
        */
        ...
        function CarregarNouExercici() {
          ...
          // ✅ CRIDA A MATHJAX DESPRÉS DE REBRE I MOSTRAR LA CORRECCIÓ
          MathJax.typesetPromise();
        }
...
function initEditor() {
  ...
  function handleInput() {
    ...
    // ✅ CRIDA A MATHJAX QUAN EL CONTINGUT DE L'EDITOR CANVIA
    MathJax.typesetPromise();
  }
